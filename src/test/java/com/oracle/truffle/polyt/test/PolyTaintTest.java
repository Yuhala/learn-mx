 ...
 * SOFTWARE.
 */
package com.oracle.truffle.polyt.test;

import java.io.IOException;
import java.util.Map;
import java.util.Set;

import com.oracle.truffle.polyt.Coverage;
import com.oracle.truffle.polyt.PolyTaint;
import com.oracle.truffle.polyt.Taint;
...
public class PolyTaintTest {
    ...
    public void exampleJSTest() throws IOException {
        ...
        // This is how we can create a context with our tool enabled if we are
        // embeddined in java
        try (Context context = Context.newBuilder("js").option(PolyTaint.ID, "true")
                .option(PolyTaint.ID + ".PrintCoverage", "false").build()) {
    ...
    // NOTE: This lookup mechanism used in this method does not work in normal
    // deployments
    ...
    // class path issolation is disabled in the pom.xml file by adding
    // -XX:-UseJVMCIClassLoader to
    // the command line.
    ...
    private static void assertJSCorrect(final Context context) {
        ...
        PolyTaint coverageInstrument = context.getEngine().getInstruments().get(PolyTaint.ID).lookup(PolyTaint.class);
        // We then use the looked up service to assert that it behaves as expected, just
        // like in any
        // other test.
        Map<com.oracle.truffle.api.source.Source, Taint> coverageMap = coverageInstrument.getCoverageMap();
        Assert.assertEquals(1, coverageMap.size());
        coverageMap.forEach((com.oracle.truffle.api.source.Source s, Taint v) -> {
    ...
    public void exampleSLTest() throws IOException {
        ...
        Assume.assumeTrue(Engine.create().getLanguages().containsKey("secureL"));
        // This is how we can create a context with our tool enabled if we are
        // embeddined in java
        try (Context context = Context.newBuilder("secureL").option(PolyTaint.ID, "true")
                .option(PolyTaint.ID + ".PrintCoverage", "false").build()) {
            Source source = Source.newBuilder("secureL", SL_SOURCE, "main").build();
            context.eval(source);
            // We can lookup services exported by the instrument, in our case this is
            // the instrument itself but it does not have to be.
            PolyTaint coverageInstrument = context.getEngine().getInstruments()
                    .get(PolyTaint.ID).lookup(PolyTaint.class);
            // We then use the looked up service to assert that it behaves as expected, just
            // like in
            // any other test.
            Map<com.oracle.truffle.api.source.Source, Taint> coverageMap = coverageInstrument.getCoverageMap();
